def getMaxPoint(pattern, cache, sticker, n):
    ...
    if n < 0:
        ...
        if n >= 3:
            cache[n] = max(getMaxPoint(pattern, cache, sticker, n-2)+sticker[int(not pattern)][n-1], getMaxPoint(pattern, cache, sticker, n-3)+sticker[int(not pattern)][n-2]) + sticker[pattern][n]
        elif n == 2:
